...
void IMU_Task_Process(IMU_t *imu)
{
    ...
    // Copy gyroscope and accelerometer data to the tx_buffer after the header
    memcpy(&tx_buffer[1], &g_imu.bmi088_raw.gyro[0], sizeof(float));
    memcpy(&tx_buffer[1 + 1 * sizeof(float)], &g_imu.bmi088_raw.gyro[1], sizeof(float));
    memcpy(&tx_buffer[1 + 2 * sizeof(float)], &g_imu.bmi088_raw.gyro[2], sizeof(float));
